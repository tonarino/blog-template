import { DatePropertyValue, RichText } from '@notionhq/client/build/src/api-types'
...
export const DateSpan: React.FC<DateProps> = ({ date, className }) => {
  ...
  return (
    <span className={className}>{getDateStr(anydate.start)}{anydate.end ? ` ⇢ ${getDateStr(anydate.end)}` : ''}</span>
  )
}
    ...
    switch (element.type) {
      ...
      case "mention":
        switch (element.mention.type) {
          case "user":
            return (
              <span key={idx}>{element.mention.user.name}</span>
            )
          case "date":
            return (
              <DateSpan key={idx} date={element.mention.date} className={`${styles.bgspan} ${styles.mentioned}`} />
            )
          case "page":
          case "database":
            return (
      ...
      default:
        ...
        return (<span key={idx} style={{ color: "gray" }}>(Unsupported rich text)</span>)
    }
  })
  return (<>{elements}</>)
}
